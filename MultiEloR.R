...
exponential_score_function <- function(n, base) {
...
# A larger base value means the scores will be more weighted towards the top finishers.
# 
...
get_new_ratings <- function(initial_ratings, result_order = c(1:length(initial_ratings)), k_value = 32, d_value = 400, score_base = 1, log_base = 10) {
...
# returns an array of updated ratings (float values) in same order as input
  ...
  expected_scores <- get_expected_scores(initial_ratings, log_base, d_value)
...
get_expected_scores <- function(initial_ratings, log_base, d_value) {
